...
export default function SignUpScreen() {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      // Clerk runtime supports this flow; generated types omit legacy verification helpers.
      await (signUp as unknown as { prepareVerification: (a: { strategy: string }) => Promise<unknown> }).prepareVerification({
        strategy: "email_code",
      });
    } catch {
  ...
  const handleVerify = async () => {
    ...
    try {
      const result = await (
        signUp as unknown as { attemptVerification: (a: { strategy: string; code: string }) => Promise<{ status: string }> }
      ).attemptVerification({ strategy: "email_code", code });
